...
Scene.fromData = function(sceneData)
{
	...
	for (var spriteData of layerData["sprites"]) 
	{
        ...
        if (!thing) {
            throw Error('thing not defined');
        }
        if (!thing.sprites) {
            throw Error('thing.sprites not defined');
        }
...
{
    var res = PIXI.Loader.shared.resources[this.spritesPath];
    if (!res) {
	throw Error("No such sprite resource: " + this.spritesPath);
    }
    if (!res.textures) {
        throw Error('res.textures is null');
    }
