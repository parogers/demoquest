 ...
 * the player. */
export function LoadingScreen(viewWidth, viewHeight)
{
...
LoadingScreen.prototype._loadSceneImages = function(dataList)
{
    ...
    PIXI.Loader.shared.defaultQueryString = "nocache=" + (new Date()).getTime();
    for (var name in dataList) {
	PIXI.Loader.shared.add(dataList[name].spritesPath);
    }
    // PIXI.Loader.shared.on("progress", (loader, res) => {
	// //console.log(res);
